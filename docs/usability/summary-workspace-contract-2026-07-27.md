...
3. current revisionの解析済みSweepは、空ではない理由を入力して既定集計から除外できる。
4. 除外済みSweepは一覧・plot上へ灰色で残し、理由を表示する。解除時は除外前の解析状態と
   メッセージへ戻す。
5. 除外・復元は`AnalysisResultStore`のmetadataだけを置換し、fitや前処理を再実行しない。
6. 表示、scope変更、行選択、drill-downでは解析を開始しない。
7. 再計算は解析ワークスペースの明示ボタンからだけ行う。
...
- 「Sweep一覧・詳細」右: 選択Sweepの4方式比較、理由付き除外・復元、「解析で確認」
- 下部: 集計条件と非再計算方針
...
- 除外操作の永続化と、操作者・時刻を含む監査履歴

現在の除外理由はアプリ実行中の`AnalysisResultStore`に保持する。測定フォルダには書き込まない。
アプリ再起動後の復元、操作者・時刻を含む監査履歴はLevel 8の結果保存形式と一緒に定義する。
